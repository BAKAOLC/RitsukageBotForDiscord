...
namespace RitsukageBot.Modules.AI
{
    ...
    {
        private async Task<PreprocessingActionData[]> TryPreprocessingMessage(string message,
        ...
        private async Task<PreprocessingActionData[]> ProgressPreprocessingActions(string json)
        {
            if (string.IsNullOrWhiteSpace(json)) return [];
            var result = new List<PreprocessingActionData>();
            try
            {
        ...
        private async Task<PreprocessingActionData> PreprocessingWebSearch(JObject? data)
        {
            ...
            return new($"Web Search: {param.Query}",
                $"[Google Search: \"{param.Query}\"]\n{string.Join("\n\n", resultStrings)}");
        }

        private static async Task<PreprocessingActionData> PreprocessingDateBaseInfo(JObject? data)
        {
            if (data is null || !data.TryGetValue("param", out var paramValue) || paramValue is not JObject paramToken)
            ...
            var todayString = today is not null
                ? $"""
                   ...
                   """
                : "未找到相关信息";
            return new($"Get Date Info: {param.Date:yyyy-MM-dd}",
                $"[Date Base Info: {param.Date:yyyy-MM-dd}]\n{todayString}");
        }

        private static async Task<PreprocessingActionData> PreprocessingRangeDateBaseInfo(JObject data)
        {
            ...
            return new($"Get Date Info: {from:yyyy-MM-dd} ~ {end:yyyy-MM-dd}", sb.ToString());
        }

        private async Task<PreprocessingActionData> PreprocessingBilibiliVideoInfo(JObject? data)
        {
            if (data is null || !data.TryGetValue("param", out var paramValue) || paramValue is not JObject paramToken)
            ...
            var info = await playerService.GetVideoPageDetailAsync(new(param.Id.ToString(), null, null))
                .ConfigureAwait(false);
            return new($"Get Bilibili Video Info: {param.Id}",
                $"[Bilibili Video Info: {param.Id}]\n{InformationStringBuilder.BuildVideoInfo(info)}");
        }

        private async Task<PreprocessingActionData> PreprocessingBilibiliUserInfo(JObject? data)
        {
            if (data is null || !data.TryGetValue("param", out var paramValue) || paramValue is not JObject paramToken)
            ...
            var info = await userService.GetUserInformationAsync(param.Id.ToString())
                .ConfigureAwait(false);
            return new($"Get Bilibili User Info: {param.Id}",
                $"[Bilibili User Info: {param.Id}]\n{InformationStringBuilder.BuildUserInfo(info)}");
        }

        private async Task<PreprocessingActionData> PreprocessingBilibiliLiveInfo(JObject? data)
        {
            if (data is null || !data.TryGetValue("param", out var paramValue) || paramValue is not JObject paramToken)
            ...
            var info = await liveService.GetLivePageDetailAsync(new(param.Id.ToString(), null, null))
                .ConfigureAwait(false);
            return new($"Get Bilibili Live Info: {param.Id}",
                $"[Bilibili Live Info: {param.Id}]\n{InformationStringBuilder.BuildLiveInfo(info)}");
        }

        private async Task<PreprocessingActionData> PreprocessingQWeatherQueryWeatherNow(JObject? data)
        {
            if (data is null || !data.TryGetValue("param", out var paramValue) || paramValue is not JObject paramToken)
            ...
            return new($"Get Weather Now: {param.Location}", sb.ToString());
        }

        private async Task<PreprocessingActionData> PreprocessingQWeatherQueryWeatherForecast(JObject? data)
        {
            if (data is null || !data.TryGetValue("param", out var paramValue) || paramValue is not JObject paramToken)
