...
namespace RitsukageBot.Modules.AI
{
    ...
    public partial class AiInteractions
    {
        ...
        private async Task<EmbedBuilder[]> ProgressActions(string json)
        {
            ...
            var results = await ProcessUnifiedActions(json, ActionPhase.Processing).ConfigureAwait(false);
            var embedResults = results.OfType<EmbedBuilder>().Where(e => e != null);
            
            return [.. embedResults];
        }

        // Note: All processing action methods have been moved to AiInteractions.UnifiedActions.cs
